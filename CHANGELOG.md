...
### Đã sửa
- Hoàn tất rà soát ảnh: audit toàn bộ 1332 từ khóa (Du lịch/Ẩm thực/Văn hóa, 190 nước). 6 từ khóa còn thiếu ảnh Wikipedia — **Dãy núi Sharr, Momo, Mì Lagman, Phô mai nâu, Đảo Sal, Bánh Quesillo** — nay đã có ảnh qua bảng `OVERRIDES` trỏ tới tên bài tin cậy.
- `Đảo Sal` đổi sang ảnh bãi biển (Santa Maria, Cape Verde) thay vì ảnh bản đồ.

### Đã thêm
- `wikiImage.js`: tầng dự phòng **Wikidata P18** (ảnh) — chỉ chạy khi tất cả các tầng Wikipedia đều trượt, nên không phát sinh request cho từ khóa đã có ảnh. Miễn phí, không cần khóa API, CORS sẵn; dựng URL ảnh qua Commons `Special:FilePath`. Tăng độ phủ cho các mục chưa có ảnh trên Wikipedia.
- Bước `summary('es')` trong chuỗi override — xử lý bài chỉ tồn tại trên Wikipedia tiếng Tây Ban Nha (vd món Quesillo của Nicaragua).

## [1.7.1] - 2026-06-20

### Đã thêm
- Bộ kiểm thử Python trong `tests/`: `test_data.py` (kiểm tra dữ liệu JSON, không cần cài gì) và `test_e2e.py` (Playwright mở từng trang, bắt lỗi Console). Hướng dẫn chạy trong README.
